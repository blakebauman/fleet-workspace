...
export class CloudflareAgentSDK implements RealAgentSDK {
	...
	ai = {
		run: async (model: string, messages: Array<{ role: string; content: string }>) => {
			try {
				...
				const isChatMessage = messages.some(
					(msg) => msg.role === 'system' && msg.content.includes('JSON format')
				)
	...
	sql = {
		exec: async (query: string, params?: any[]) => {
			try {
				// This method should be called from within the Durable Object context
				// The actual SQL execution will be handled by the Durable Object's storage.sql
				// This is a proxy that forwards to the Durable Object's SQL implementation
				console.log('SQL Query:', query, 'Params:', params)

				// Note: This method should be called from within the Durable Object context
				// where this.sqlStorage.exec() is available. The actual implementation
				// happens in the Durable Object's sql() method.
				throw new Error(
					'SQL execution must be called from within Durable Object context. Use this.sql() method instead.'
				)
			} catch (error) {
	...
	schedule = {
		create: async (name: string, data: any, options?: { delay?: number }) => {
			try {
				...
				throw new Error(
					`Failed to schedule workflow ${name}: ${error instanceof Error ? error.message : 'Unknown error'}`
				)
			}
		},
	}
